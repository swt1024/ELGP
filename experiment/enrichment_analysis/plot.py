# We made some improvements to the GSEA source code. 
# You need to replace the file at your_environment_path\Lib\site-packages\gseapy\plot.py
...
class GSEAPlot(object):
    def __init__(
        ...
        **kwargs,
        ...
        if self.module == "ssgsea":
            ...
            self._nes_label = "ES: " + "{:.3f}".format(float(es))
        ...
        self.fig.suptitle(self.term, fontsize=8, wrap=True)
    ...
    def axes_rank(self, rect):
        ...
        if self.module == "ssgsea":
            ...
            ax1.set_ylabel("log ranked metric", fontsize=8)
        else:
            ax1.fill_between(self._x, y1=self.rankings, y2=0, color="#C9D3DB")
            ax1.set_ylabel("Ranked metric", fontsize=8)
        ...
        ax1.set_xlabel("Gene rank", fontsize=8)
    ...
    def axes_hits(self, rect, bottom: bool = False):
        ...
        if bottom:
            ax2.set_xlabel("Gene rank", fontsize=8)
    ...
    def axes_stat(self, rect):
        ...
        ax4.text(0.2, 0.3, self._nes_label, transform=ax4.transAxes, fontsize=8)
        ...
        ax4.set_ylabel("Enrichment score", fontsize=8)
